...
# ── Style CSS cohérent avec app.py et US-01 ──────────────────────────────────
st.markdown("""
    <style>
        :root {
            --green-dark:   #1B5E20;
...
with st.form("formulaire_annonce", clear_on_submit=False):
    ...
    description = st.text_area(
        ...
        placeholder="Décrivez la qualité, la disponibilité ou d’autres détails sur votre produit...",
        height=120
    )

    photos = st.file_uploader(
        "Ajouter de 1 à 3 photos *",
        type=["jpg", "jpeg", "png"],
        accept_multiple_files=True,
        help="Vous pouvez importer jusqu’à 3 images"
    )

    bouton_publier = st.form_submit_button(
        "📢 Publier l'annonce",
...
if bouton_publier:
    ...
    if not photos or len(photos) == 0:
        erreurs.append("Vous devez ajouter au moins une photo.")
    elif len(photos) > 3:
        erreurs.append("Vous ne pouvez pas ajouter plus de 3 photos.")
    ...
    if erreurs:
        ...
        <div class="success-box">
            ...
            Votre produit <strong>{produit.strip()}</strong> est maintenant visible sur AgroSénégal.
        </div>
        """, unsafe_allow_html=True)
